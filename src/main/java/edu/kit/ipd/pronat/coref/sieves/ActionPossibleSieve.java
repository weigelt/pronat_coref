...
public class ActionPossibleSieve extends Sieve {
	...
	/*
	 * private Set<State> determineStates(PronounEntity pronounEntity, Set<State>
	 * states) { Set<State> currentStates = new HashSet<>(states); if
	 * (pronounEntity.hasRelationsOfType(ReferentRelation.class)) { List<Relation>
	 * relations = pronounEntity.getRelationsOfType(ReferentRelation.class); for
	 * (Relation relation : relations) { ReferentRelation refRel =
	 * (ReferentRelation) relation; if (refRel.getEnd().equals(pronounEntity)) { if
	 ...
	 * (ActionEntityRelation aeRel : aeRels) { Action action = aeRel.getAction();
	 * 
	 ...
	 * }
	 * 
	 * } } currentStates = determineStates(pEntity, currentStates); } } } } } return
	 * currentStates; }
	 */

	/*
	 * private Set<State> determineStates(ObjectEntity objEntity, Set<State> states)
	 * { Set<State> currentStates = new HashSet<>(states); if

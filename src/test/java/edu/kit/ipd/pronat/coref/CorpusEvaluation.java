...
import edu.kit.ipd.pronat.coref.util.Text;
		...
		for (String id : texts.keySet()) {
			if ((!id.startsWith("s")) && ((id.split("\\.")[0].equalsIgnoreCase("if") && Integer.parseInt(id.split("\\.")[2]) >= 10)
				...
				try {
					...
					coref.exec();
					Context context = coref.getContext();
					CorefTestHelper.TestResult result = CorefTestHelper.checkCorefChains(context, expected);
					tp += result.result[0];
	...
	public void evalMultipleNew() {
		...
		for (String id : texts.keySet()) {
			...
			try {
				...
				CorefTestHelper.TestResult result = CorefTestHelper.checkCorefChains(cResult, expected);
				tp += result.result[0];
				overall += expected.size();
				tooMuch += result.result[1];
				wrong += result.result[2];
